...
new_area(int size)
{
        struct area *area = malloc(sizeof (struct area) +
                                   size * size * sizeof area->squares[0]);
        area->size = size;
        return area;
}
...
static void
dump_history(size_t history_size,
             struct area **history)
{
        for (size_t i = 0; i < history_size; i++) {
                const struct area *area = history[i];
                char filename[128];

                sprintf(filename, "frame-%08lu.ppm", i);

                FILE *out = fopen(filename, "wb");

                fprintf(out, "P6\n%i %i\n255\n", area->size, area->size);

                for (int x = 0; x < area->size * area->size; x++) {
                        enum acre_state state = area->state[x];
                        const char *colour = "\0\0\0";
                        switch (state) {
                        case ACRE_STATE_OPEN:
                                break;
                        case ACRE_STATE_TREES:
          ...
          unsigned long n_steps)
{
        ...
        for (unsigned long minute = 0; minute < n_steps; minute++) {
                ...
                if (find_history(history_length - 1,
                        ...
                        dump_history(history_length - 1 - repeat_pos,
                                     history + repeat_pos);
                        break;
                }
